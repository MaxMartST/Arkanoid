...
const string MUSIC{ "music/we.ogg" };
...
{
	Music music;
	void loadMusicGames(const string &path)
	{
...
void updateObjects(World &mWorld, Progress &mProgress, float myDeltaTime)
{
	...
	for (auto &brick : mWorld.bricks)
	{
		...
		updatePoints(brick, mWorld.ball, mProgress); //обнавляем данные очков
	}
...
void initObjects(World &myWorld, GameTextures &myTextures)
{
	...
	myWorld.musicGames.loadMusicGames(MUSIC);
}
